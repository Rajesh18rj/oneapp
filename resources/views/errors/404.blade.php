                ...
                <div class="col-sm-6 col-md-4 text-center text-sm-left">
                    ...
                    {{-- @if (auth('vendor')->check())
                        <a class="btn btn-primary"
                            href="{{ route('vendor.dashboard') }}">{{ translate('messages.dashboard') }}</a>
                    @else
                        <a class="btn btn-primary"
                            href="{{ route('admin.dashboard') }}">{{ translate('messages.dashboard') }}</a>
                    @endif --}}
                </div>
            </div>
            <!-- End Row -->
        ...
        <ul class="list-inline list-separator">
            <li class="list-inline-item">
                <a class="list-separator-link" target="_blank"
                    href="{{ route('contact-us') }}">{{ \App\Models\BusinessSetting::where(['key' => 'business_name'])->first()->value ?? 'Stack Food' }}
