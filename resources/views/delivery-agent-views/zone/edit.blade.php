    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title text-capitalize"><i class="tio-edit"></i> {{ translate('messages.zone') }}
                        {{ translate('messages.update') }}</h1>
                </div>
            </div>
        </div>
        <!-- End Page Header -->
        <div class="row gx-2 gx-lg-3">
            <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
                <form action="{{ route('delivery-agent.zone.update', $zone->id) }}" method="post" id="zone_form">
                    @csrf
                    <div class="row">
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" class="form-control"
                                    placeholder="{{ translate('messages.new_zone') }}" value="{{ $zone->name }}" required>
                            </div>
                            <div class="form-group">
                                <label class="input-label" for="exampleFormControlInput1">Sub Zone
                                    {{ translate('messages.name') }}</label>
                                <input type="text" name="sub_zone" class="form-control"
                                    placeholder="{{ translate('messages.new_sub_zone') }}" value="{{ $zone->sub_zone }}"
                                    required>
                            </div>
                            ...
                            <div id="sub_zone_data">
                                @php

                                    $sub_zone_distances = explode(',', $zone->sub_zone_distance);

                                @endphp

                                <input type="hidden" id="total_sub_zones" value="{{ count($sub_zone_distances) }}">
                                @foreach ($sub_zone_distances as $key => $value)
                                    <div class="form-group">
                            ...
                            <div class="form-group">
                                <label class="input-label" for="exampleFormControlInput1">Coordinates<span
...
<?php if(count($zone->coordinates[0]) != $key+1) {if($key != 0) echo(','); ?>({{ $coords->getLat() }}, {{ $coords->getLng() }})<?php } ?>
@endforeach
</textarea>
                            </div>
                        </div>
...
@push('script_2')
    <script
        src="https://maps.googleapis.com/maps/api/js?v=3.45.8&key={{ \App\Models\BusinessSetting::where('key', 'map_api_key')->first()->value }}&libraries=drawing,places">
    </script>
    <script>
        auto_grow();
    ...
    <script>
        ...
        function initialize() {
            var myLatlng = new google.maps.LatLng({{ trim(explode(' ', $zone->center)[1], 'POINT()') }},
                {{ trim(explode(' ', $zone->center)[0], 'POINT()') }});
