        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title"><i class="tio-filter-list"></i> {{ translate('messages.stores') }} <span
                        ...
                        <table id="columnSearchDatatable"
                            ...
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th class="text-uppercase" style="width: 10%;">{{ translate('messages.featured') }}
                                    </th>
                            ...
                            <tbody id="set-rows">
                                @foreach ($stores as $key => $store)
                                    <tr class="table_row" data-id="{{ $store->id }}">
                                        <td>
                                        ...
                                        <td>
                                            <label class="toggle-switch toggle-switch-sm"
                                                ...
                                                <input type="checkbox"
                                                    onclick="location.href='{{ route('admin.vendor.featured', [$store->id, $store->featured ? 0 : 1]) }}'"
                                                    class="toggle-switch-input" id="featuredCheckbox{{ $store->id }}"
                                            ...
                                            @if (isset($store->vendor->status))
                                                @if ($store->vendor->status)
                                                    <label class="toggle-switch toggle-switch-sm"
                                                        ...
                                                        <input type="checkbox"
                                                            onclick="status_change_alert('{{ route('admin.vendor.status', [$store->id, $store->status ? 0 : 1]) }}', '{{ translate('messages.you_want_to_change_this_store_status') }}', event)"
                                        ...
                                        <td>
                                            <a class="btn btn-sm btn-white"
                                                href="{{ route('admin.vendor.view', [$store['id']]) }}"
                                                title="{{ translate('messages.view') }} {{ translate('messages.store') }}"><i
                                                    class="tio-visible text-success"></i>
                                            </a>
                                            <a class="btn btn-sm btn-white"
                                                href="{{ route('admin.vendor.edit', [$store['id']]) }}"
                                                title="{{ translate('messages.edit') }} {{ translate('messages.store') }}"><i
                                                    class="tio-edit text-primary"></i>
                                            </a>
                                            <a class="btn btn-sm btn-white" href="javascript:"
                                                onclick="form_alert('vendor-{{ $store['id'] }}','{{ translate('You want to remove this store') }}')"
                                                title="{{ translate('messages.delete') }} {{ translate('messages.store') }}"><i
                                                    class="tio-delete-outlined text-danger"></i>
                                            </a>
            ...
            $('#bulkDelete').on('click', function() {
                ...
                if (itemIds.length > 0) {
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            $.ajax({
                                url: '{{ url('/') }}/admin/vendor/bulk-item-delete',
                                type: 'DELETE',
                                data: {
                                    item_ids: itemIds,
                                    "_token": "{{ csrf_token() }}"
