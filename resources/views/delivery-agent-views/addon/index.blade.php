    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title">{{ translate('messages.addon') }}</h1>
                </div>
                @if (isset($addon))
                    <a href="{{ route('admin.addon.add-new') }}" class="btn btn-primary pull-right"><i
                            class="tio-add-circle"></i> {{ translate('messages.add') }} {{ translate('messages.new') }}
                        {{ translate('messages.addon') }}</a>
                @endif
            </div>
        </div>
        <!-- End Page Header -->
        ...
        <div class="card">
            <div class="card-header">
                <h5>{{ translate('messages.add') . ' ' . translate('messages.new') }} {{ translate('messages.addon') }}</h5>
            </div>
            <div class="card-body">
                <form
                    action="{{ isset($addon) ? route('admin.addon.update', [$addon['id']]) : route('admin.addon.store') }}"
                    method="post">
                    @csrf
                    @if ($language)
        ...
        <div class="card mt-1">
            <div class="card-header">
                <h5 class="card-header-title"> {{ translate('messages.addon') }} {{ translate('messages.list') }}<span
                        class="badge badge-soft-dark ml-2" id="itemCount">{{ $addons->total() }}</span></h5>
                        ...
                        <form id="search-form">
                            ...
                            <div class="input-group input-group-merge input-group-flush">
                                ...
                                <input id="datatableSearch" type="search" name="search" class="form-control"
                                    placeholder="{{ translate('messages.search_addons') }}" aria-label="Search addons">
                            </div>
                            <!-- End Search -->
                        </form>
                    </div>
                    ...
                    <table id="datatable"
                        ...
                        <thead class="thead-light">
                            <tr>
                                ...
                                <th style="width: 20%">{{ translate('messages.price') }}</th>
                                <th style="width: 30%">{{ translate('messages.store') }}</th>
                        ...
                        <tbody id="set-rows">
                            @foreach ($addons as $key => $addon)
                                <tr class="table_row" data-id="{{ $addon->id }}">
                                    <td>
                                    ...
                                    <td>{{ Str::limit($addon->store ? $addon->store->name : translate('messages.store') . ' ' . translate('messages.deleted'), 25, '...') }}
                                    </td>
                                    <td>
                                        <label class="toggle-switch toggle-switch-sm"
                                            for="stausCheckbox{{ $addon->id }}">
                                            <input type="checkbox"
                                                onclick="location.href='{{ route('admin.addon.status', [$addon['id'], $addon->status ? 0 : 1]) }}'"class="toggle-switch-input"
                                                id="stausCheckbox{{ $addon->id }}"
                                    ...
                                    <td>
                                        <a class="btn btn-sm btn-white"
                                            ...
                                            title="{{ translate('messages.edit') }} {{ translate('messages.addon') }}"><i
                                                class="tio-edit"></i></a>
                                        <a class="btn btn-sm btn-white" href="javascript:"
                                            onclick="form_alert('addon-{{ $addon['id'] }}','Want to delete this addon ?')"
                                            title="{{ translate('messages.delete') }} {{ translate('messages.addon') }}"><i
            ...
            $('#bulkDelete').on('click', function() {
                ...
                if (itemIds.length > 0) {
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            $.ajax({
                                url: '{{ url('/') }}/admin/addon/bulk-item-delete',
                                type: 'DELETE',
                                data: {
                                    item_ids: itemIds,
                                    "_token": "{{ csrf_token() }}"
