    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                ...
                @if (isset($category))
                    <a href="{{ route('admin.category.add') }}" class="btn btn-primary pull-right"><i
                            class="tio-add-circle"></i> {{ translate('messages.add') }} {{ translate('messages.new') }}
                        {{ translate('messages.category') }}</a>
                @endif
            </div>
        </div>
        <!-- End Page Header -->
        ...
        <div class="card">
            <div class="card-header">
                <h5>{{ isset($category) ? translate('messages.update') : translate('messages.add') . ' ' . translate('messages.new') }}
                    {{ translate('messages.category') }}</h5>
            </div>
            <div class="card-body">
                <form
                    ...
                    <div class="form-group">
                        <label>{{ translate('messages.image') }}</label><small style="color: red">* (
        ...
        <div class="card mt-3">
            <div class="card-header pb-2">
                <h5 class="col">{{ translate('messages.category') }} {{ translate('messages.list') }}<span
                    ...
                    <table id="columnSearchDatatable"
                        ...
                        <tbody id="table-div">
                            @foreach ($categories as $key => $category)
                                <tr class="table_row" data-id="{{ $category->id }}">
                                    <td>
                                    ...
                                    <td>
                                        <form action="{{ route('admin.category.priority', $category->id) }}">
                                            <select name="priority" id="priority" class="w-100"
            ...
            $('#bulkDelete').on('click', function() {
                ...
                if (itemIds.length > 0) {
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            $.ajax({
                                url: '{{ url('/') }}/admin/category/bulk-item-delete',
                                type: 'DELETE',
                                data: {
                                    item_ids: itemIds,
                                    "_token": "{{ csrf_token() }}"
