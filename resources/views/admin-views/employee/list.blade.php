...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="d-md-flex_ align-items-center justify-content-between mb-2">
            <div class="row">
                <div class="col-md-8">
                    <h3 class="h3 mb-0 text-black-50">{{ translate('messages.Employee') }} {{ translate('messages.list') }}
                    </h3>
                </div>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header py-0">
                        <h5>{{ translate('messages.Employee') }} {{ translate('messages.table') }} <span
                                class="badge badge-soft-dark ml-2" id="itemCount">{{ $em->total() }}</span></h5>
                        <form action="javascript:" id="search-form">
                            ...
                            <div class="input-group input-group-merge input-group-flush">
                                ...
                                <input id="datatableSearch_" type="search" name="search" class="form-control"
                                    placeholder="{{ translate('messages.search') }}" aria-label="Search">
                    ...
                    <div class="card-body" style="padding: 0">
                        ...
                        <div class="table-responsive">
                            <table id="datatable"
                                ...
                                <tbody id="set-rows">
                                    @foreach ($em as $k => $e)
                                        <tr data-id="{{ $e->id }}" class="table_row">
                                            <td>
                                            ...
                                            <td>{{ $e['phone'] }}</td>
                                            <td>{{ $e->role ? $e->role['name'] : 'N/A' }}</td>
                                            <td>
                                                <a class="btn btn-sm btn-white"
                                                    href="{{ route('admin.employee.edit', [$e['id']]) }}"
                                                    title="{{ translate('messages.edit') }} {{ translate('messages.Employee') }}"><i
                                                        class="tio-edit"></i>
                                                </a>
                                                <a class="btn btn-sm btn-danger" href="javascript:"
                                                    onclick="form_alert('employee-{{ $e['id'] }}','{{ translate('messages.Want_to_delete_this_role') }}')"
                                                    title="{{ translate('messages.delete') }} {{ translate('messages.Employee') }}"><i
            ...
            $('#bulkDelete').on('click', function() {
                ...
                if (itemIds.length > 0) {
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            $.ajax({
                                url: '{{ url('/') }}/admin/employee/bulk-item-delete',
                                type: 'DELETE',
                                data: {
                                    item_ids: itemIds,
                                    "_token": "{{ csrf_token() }}"
