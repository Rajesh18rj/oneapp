        ...
        <div class="row gx-2 gx-lg-3">
            <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
                <form action="{{ route('admin.vendor.update', [$store['id']]) }}" method="post" class="js-validate"
                    ...
                    @csrf
                    <small class="nav-subtitle text-secondary border-bottom">{{ translate('messages.store') }}
                        ...
                        <div class="col-md-6 col-12">
                            ...
                            <div class="form-group">
                                ...
                                <textarea type="text" name="address" class="form-control"
                                    placeholder="{{ translate('messages.store') }} {{ translate('messages.address') }}" required>{{ $store->address }}</textarea>
                            </div>
                            <div class="form-group">
                    ...
                    <div class="row">
                        <div class="col-md-4 col-12">
                                ...
                                <select name="module_id" required class="form-control js-select2-custom"
                                    ...
                                    @foreach (\App\Models\Module::notParcel()->get() as $module)
                                        <option value="{{ $module->id }}"
                                            {{ $store->module_id == $module->id ? 'selected' : '' }}>
                                            {{ $module->module_name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select name="zone_id" id="choice_zones" onchange="get_zone_data(this.value)"
                                    ...
                                    @foreach (\App\Models\Zone::active()->get() as $zone)
                                        ...
                                        @else
                                            <option value="{{ $zone->id }}"
                                                {{ $store->zone_id == $zone->id ? 'selected' : '' }}>{{ $zone->name }}
                                            </option>
                                        @endif
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                <label class="input-label" for="choice_sub_zones">Sub Zones<span
                                        class="input-label-secondary"
                                        title="{{ translate('messages.select_zone_for_map') }}"><img
                                            src="{{ asset('/public/assets/admin/img/info-circle.svg') }}"
                                            alt="{{ translate('messages.select_zone_for_map') }}"></span></label>
                                ...
                                @php
                                    $zoneData = \App\Models\Zone::where('id', $store->zone_id)->first();
                                    $zoneName = $zoneData ? $zoneData->name : '';
                                @endphp


                                <select name="sub_zone_name" id="choice_sub_zones" required
                                    class="form-control js-select2-custom" data-placeholder="Sub Zones">
                                    <option value="" selected disabled>{{ translate('messages.select') }}
                                        {{ translate('messages.zone') }}</option>

                                    @foreach (\App\Models\Zone::where('name', $zoneName)->get() as $zone)
                                        <option value="{{ $zone->sub_zone }}"
                                            {{ $store->sub_zone_name == $zone->sub_zone ? 'selected' : '' }}>
                                            {{ $zone->sub_zone }}</option>
                                    @endforeach
                    ...
                    <div class="form-group">
                        <label for="name">{{ translate('messages.upload') }} {{ translate('messages.cover') }}
                            {{ translate('messages.photo') }} <span
                                class="text-danger">({{ translate('messages.ratio') }} 2:1)</span></label>
                        <div class="custom-file">
                    ...
                    <div class="row">
                        <div class="col-md-6 col-12">
                            <div class="form-group">
                                <label class="input-label" for="gst_number">{{ translate('messages.gst_number') }}<span
                                        class="input-label-secondary"
                                        title="{{ translate('messages.gst_number') }}"></label>
                                <input type="text" id="gst_number" name="gst_number" class="form-control"
                                    placeholder="{{ translate('messages.gst_number') }}"
                                    value="{{ $store->gst_number }}" required>
                            </div>
                        </div>
                        <div class="col-md-6 col-12 fssai_number_section">
                            <div class="form-group">
                                <label class="input-label"
                                    for="fssai_number">{{ translate('messages.fssai_number') }}<span
                                        class="input-label-secondary"
                                        title="{{ translate('messages.fssai_number') }}"></label>
                                <input type="text" id="fssai_number" name="fssai_number" class="form-control"
                                    placeholder="{{ translate('messages.fssai_number') }}"
                                    value="{{ $store->fssai_number }}" required>
                            </div>
                        </div>

                        <div class="col-md-6 col-12 drug_license_number_section d-none">
                            <div class="form-group">
                                <label class="input-label"
                                    for="drug_license_number">{{ translate('messages.drug_license_number') }}<span
                                        class="input-label-secondary"
                                        title="{{ translate('messages.drug_license_number') }}"></label>
                                <input type="text" id="drug_license_number" name="drug_license_number"
                                    class="form-control" placeholder="{{ translate('messages.drug_license_number') }}"
                    ...
                    <small class="nav-subtitle text-secondary border-bottom">{{ translate('messages.login') }}
                        {{ translate('messages.info') }}</small>
                    <br>
                    <div class="row">
                        <div class="col-md-4 col-12">
                            <div class="form-group">
                                ...
                                <input type="email" name="email" class="form-control"
                                    placeholder="Ex : ex@example.com" value="{{ $store->email }}" required>
                            </div>
                        </div>
                        <div class="col-md-4 col-12">
                            <div class="js-form-message form-group">
                                ...
                                <div class="input-group input-group-merge">
                                    <input type="password" class="js-toggle-password form-control" name="password"
                                        id="signupSrPassword"
                                        placeholder="{{ translate('messages.password_length_placeholder', ['length' => '9+']) }}"
                                        aria-label="9+ characters required"
...
@push('script_2')
    <script>
        ...
        $("#module_id").on("change", function() {
            if ($(this).val() == 3) {
                $(".fssai_number_section").removeClass("d-none")
                $(".drug_license_number_section").addClass("d-none");
                $("#fssai_number").prop("required", "required");
                $("#drug_license_number").prop("required", "");
            } else if ($(this).val() == 4) {
                $(".fssai_number_section").addClass("d-none")
                $(".drug_license_number_section").removeClass("d-none");
                $("#fssai_number").prop("required", "");
                $("#drug_license_number").prop("required", "required");
            } else {
                $(".fssai_number_section").removeClass("d-none")
                $(".drug_license_number_section").addClass("d-none");
                $("#fssai_number").prop("required", "required");
