...
@section('title', 'Commodity Type List')
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-sm-4 col-12">
                    <h1 class="page-header-title"> {{ translate('messages.commodity_Type') }}
                        {{ translate('messages.list') }}<span class="badge badge-soft-dark ml-2"
                            id="foodCount">{{ $commodity_types->total() }}</span></h1>
                </div>
                    ...
                    <div class="card">
                        ...
                        <div class="table-responsive datatable-custom" id="table-div" style="margin-top:20px;">
                            ...
                            <table id="datatable"
                                ...
                                <tbody id="set-rows">
                                    @foreach ($commodity_types as $key => $item)
                                        <tr data-id="{{ $item->id }}" class="table_row">
                                            <td>
                                                <input type="checkbox" class="row-checkbox">
                                            </td>
                                            <td>{{ $key + $commodity_types->firstItem() }}</td>
                                            <td>
                                                {{ $item->name }}
                                            </td>
                                            <td>
                                                <a class="btn btn-sm btn-white"
                                                    href="{{ route('admin.commodity_type.edit', [$item['id']]) }}"
                                                    title="{{ translate('messages.edit') }} {{ translate('messages.commodity type') }}"><i
                                                        class="tio-edit"></i>
                                                </a>
                                                <a class="btn btn-sm btn-white" href="javascript:"
                                                    onclick="form_alert('food-{{ $item['id'] }}','{{ translate('messages.Want_to_delete_this_commodity_type?') }}')"
                                                    title="{{ translate('messages.delete') }} {{ translate('messages.commodity type') }}"><i
                                                        class="tio-delete-outlined"></i>
                                                </a>
                                                <form action="{{ route('admin.commodity_type.delete', [$item['id']]) }}"
                            ...
                            <div class="page-area">
                                <table>
                                    <tfoot class="border-top">
                                        {!! $commodity_types->withQueryString()->links() !!}
                                    </tfoot>
                                </table>
                            </div>
                        </div>
                ...
                $('#bulkDelete').on('click', function() {
                    ...
                    if (commodityIds.length > 0) {
                        ...
                        Swal.fire({
                            ...
                            text: "Want to delete this commodity types ?",
                            type: 'warning',
                            ...
                            if (result.value) {
                                $.ajax({
                                    url: '{{ url('/') }}/admin/commodity-type/bulk-item-delete',
                                    type: 'DELETE',
                                    data: {
                        ...
                        alert("Please select at least one commodity type to delete.");
                    }
