            ...
            <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
                <form action="javascript:" method="post" id="product_form"
                    ...
                    <div class="row" style="margin-top:20px">
                        ...
                        <div class="col-sm-3 col-6">
                            <div class="form-group">
                                ...
                                <input type="number" name="tax" class="form-control" placeholder="{{translate('messages.vat/tax')}}" min="0"
                                    step=".01" required value="{{$product['tax']}}">
                            </div>
                        </div>

                       <div class="col-sm-3 col-6">
                            <div class="form-group">
                                <label class="input-label">{{translate('HSN')}}</label>
                                <input type="text" name="hsn" class="form-control" placeholder="{{translate('HSN')}}"
                                    value="{{$product['hsn']}}" />
                            </div>
                        </div>
                         ...
                         <div class="col-sm-3 col-6">
                            <div class="form-group">
                                <label class="input-label">Upload {{ translate('HSN') }} Image</label>
                                <input type="file" name="hsn_image" id="hsn_image" />
                            </div>
                            <?php
                              if($product['hsn_image'] != 'def.png'){
                              
                              ?>
                            <center id="hsn-image-viewer-section" class="pt-2">
                               <img style="width:100%;height: 200px;border: 1px solid; border-radius: 10px;" id="hsn_image_viewer"
                                 src="{{asset('storage/app/public/product')}}/{{$product['hsn_image']}}" alt="banner image" />
                            </center>
                            
                            <?php } else { ?>
                              <center style="display: none" id="hsn-image-viewer-section" class="pt-2">
                               <img style="width:100%;height: 200px;border: 1px solid; border-radius: 10px;" id="hsn_image_viewer"
                        ...
                        <div class="col-sm-4 col-6">
                            <div class="form-group">
                                <label class="input-label">{{translate('SAC')}}</label>
                                <input type="text" name="sac" class="form-control" placeholder="{{translate('SAC')}}"
                                    value="{{$product['sac']}}" />
                            </div>
                        </div>
                    ...
                    <div class="row"  id="time_input">
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                <label class="input-label" for="exampleFormControlInput1">Video (Youtube Url)</label>
                                <input type="time" value="{{$product['video_url']}}" name="video_url" class="form-control" />
                            </div>
                        </div>
                   ...
                   <?php $videoStatus = 0; ?>
                    ...
                    @foreach ($product->images as $image)
                        @if(str_contains($image, 'video-'))
                         <?php $videoStatus = 1; ?>
                        <div class="form-group">
                            <label class="input-label" for="exampleFormControlInput1">Video (Youtube Url)</label>
                            <input type="text"  name="video_url" class="form-control" value="{{ ltrim($image, 'video-') }}" />
                        </div>
                        @endIf
                    @endforeach
                    ...
                    @endIf 
                    	...
                    	<div class="form-group">
                                <label class="input-label" for="batch_number">{{translate('messages.batch_number')}}<span
                                        class="input-label-secondary">*</span></label>
                                <input type="text"  name="batch_number" class="form-control" value="{{$product['batch_number']}}"  required/>
                            </div>
                            
                              <div class="form-group">
                        <label class="input-label" for="expiry_date">{{translate('messages.expiry_date')}}<span
                                class="input-label-secondary">*</span></label>
                        <input type="date"  name="expiry_date" id="expiry_date"  value="{{$product['expiry_date']}}" class="form-control" required/>
                    </div>
...
@push('script_2')
    <script>
        ...
        function readURL(input) {
          ...
          function previewHSNImage(input) {
            if (input.files && input.files[0]) {
                var reader = new FileReader();

                reader.onload = function(e) {
                    $('#hsn_image_viewer').attr('src', e.target.result);
                }
        ...
        $("#customFileEg1").change(function () {
          ...
          $("#hsn_image").change(function() {
            previewHSNImage(this);
            $('#hsn-image-viewer-section').show(1000);
        });
    ...
    <script>
        ...
        $('#category_id').select2({
            ajax: {
                url: '{{url('/')}}/vendor-panel/item/get-categories?parent_id=0',
                data: function (params) {
                    return {
                        q: params.term, // search term
                        page: params.page,
        ...
        $('#sub-categories').select2({
            ajax: {
                url: '{{url('/')}}/vendor-panel/item/get-categories',
                data: function (params) {
                    return {
                        q: params.term, // search term
                        page: params.page,
            ...
            $.post({
                ...
                success: function (data) {
                    ...
                    if (data.errors) {
                        ...
                        setTimeout(function () {
                            location.href = '{{\Request::server('HTTP_REFERER')??route('vendor.item.list')}}';
                        }, 2000);
                    }
                }
            });
