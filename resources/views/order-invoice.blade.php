   ...
   <div class="outer_border">
      <div class="row">
         <div  class=" pull-left top_box p-4">
            ...
            <h2 class="text_color1" style="font-size:20px">{{ $order->store ? $order->store->name : 'N/A' }}</h2> 

           {{ $order->store ? ucfirst($order->store->address) : 'N/A' }}
           <br>
           <p>
                Phone : {{ $order->store ? $order->store->phone : 'N/A' }} <br>
                Email : {{ $order->store ? $order->store->email : 'N/A' }} <br>
                GST Number : {{ $order->store ? $order->store->gst_number : 'N/A' }} <br>
                
                @if($order->store->module_id != '4')
                     FSSAI  Number : {{ $order->store ? $order->store->fssai_number : 'N/A' }} <br>
                @endif
           
               
               @if($order->store->module_id == '4')
                Drug License Number : {{ $order->store ? $order->store->drug_license_number : 'N/A' }} <br>
                @endif
         </p>
