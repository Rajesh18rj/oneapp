...
class BusinessSettingsController extends Controller
{
    ...
    public function business_setup(Request $request)
    {
  	...
  	DB::table('business_settings')->updateOrInsert(['key' => 'min_delivery_man_amount'], [
            'value' => $request['min_delivery_man_amount']
        ]);

        DB::table('business_settings')->updateOrInsert(['key' => 'channel_partner_commission'], [
            'value' => $request['channel_partner_commission']
        ]);

        DB::table('business_settings')->updateOrInsert(['key' => 'deliveryman_deposit_amount'], [
            'value' => $request['deliveryman_deposit_amount']
        ]);

        DB::table('business_settings')->updateOrInsert(['key' => 'deliveryman_deposit'], [
            'value' => $request['deliveryman_deposit']
        ]);

        DB::table('business_settings')->updateOrInsert(['key' => 'delivery_man_commission'], [
            'value' => $request['delivery_man_commission']
        ]);
        ...
        $gst_certificate = BusinessSetting::where(['key' => 'gst_certificate'])->first();
        if ($request->has('gst_certificate')) {

            $gst_certificate =  Helpers::upload('business/', 'png', $request->file('gst_certificate'));
        } else {
            $gst_certificate = ($gst_certificate['value']) ?? '';
        }

        DB::table('business_settings')->updateOrInsert(['key' => 'gst_certificate'], [
            'value' => $gst_certificate
        ]);

        $fssai_certificate = BusinessSetting::where(['key' => 'fssai_certificate'])->first();
        if ($request->has('fssai_certificate')) {

            $fssai_certificate =  Helpers::upload('business/', 'png', $request->file('fssai_certificate'));
        } else {
            $fssai_certificate = ($fssai_certificate['value']) ?? '';
        }

        DB::table('business_settings')->updateOrInsert(['key' => 'fssai_certificate'], [
            'value' => $fssai_certificate
        ]);

        $pan_card = BusinessSetting::where(['key' => 'pan_card'])->first();
        if ($request->has('pan_card')) {
            $pan_card =  Helpers::upload('business/', 'png', $request->file('pan_card'));
        } else {
            $pan_card = ($pan_card['value']) ?? '';
        }

        DB::table('business_settings')->updateOrInsert(['key' => 'pan_card'], [
            'value' => $pan_card
        ]);

        $tfn_certificate = BusinessSetting::where(['key' => 'tfn_certificate'])->first();
        if ($request->has('tfn_certificate')) {

            $tfn_certificate =  Helpers::upload('business/', 'png', $request->file('tfn_certificate'));
        } else {
            $tfn_certificate = ($tfn_certificate['value']) ?? '';
        }

        DB::table('business_settings')->updateOrInsert(['key' => 'tfn_certificate'], [
            'value' => $tfn_certificate
        ]);

        $acn_certificate = BusinessSetting::where(['key' => 'acn_certificate'])->first();
        if ($request->has('acn_certificate')) {

            $acn_certificate =  Helpers::upload('business/', 'png', $request->file('acn_certificate'));
        } else {
            $acn_certificate = ($acn_certificate['value']) ?? '';
        }

        DB::table('business_settings')->updateOrInsert(['key' => 'acn_certificate'], [
            'value' => $acn_certificate
        ]);

        $abn_certificate = BusinessSetting::where(['key' => 'abn_certificate'])->first();
        if ($request->has('abn_certificate')) {

            $abn_certificate =  Helpers::upload('business/', 'png', $request->file('abn_certificate'));
        } else {
            $abn_certificate = ($abn_certificate['value']) ?? '';
        }

        DB::table('business_settings')->updateOrInsert(['key' => 'abn_certificate'], [
            'value' => $abn_certificate
        ]);


        $ein_certificate = BusinessSetting::where(['key' => 'ein_certificate'])->first();
        if ($request->has('ein_certificate')) {

            $ein_certificate =  Helpers::upload('business/', 'png', $request->file('ein_certificate'));
        } else {
            $ein_certificate = ($ein_certificate['value']) ?? '';
        }

        DB::table('business_settings')->updateOrInsert(['key' => 'ein_certificate'], [
            'value' => $ein_certificate
        ]);


        $cin_certificate = BusinessSetting::where(['key' => 'cin_certificate'])->first();
        if ($request->has('cin_certificate')) {

            $cin_certificate =  Helpers::upload('business/', 'png', $request->file('cin_certificate'));
        } else {
            $cin_certificate = ($cin_certificate['value']) ?? '';
        }

        DB::table('business_settings')->updateOrInsert(['key' => 'cin_certificate'], [
            'value' => $cin_certificate
        ]);

	DB::table('business_settings')->updateOrInsert(['key' => 'deposit_note'], [
            'value' => $request['deposit_note']
        ]);


        DB::table('business_settings')->updateOrInsert(['key' => 'courier_note'], [
            'value' => $request['courier_note']
        ]);
        
         DB::table('business_settings')->updateOrInsert(['key' => 'cookie_text'], [
            'value' => $request['cookie_text']
        ]);
        
         DB::table('business_settings')->updateOrInsert(['key' => 'is_gst_certificate_enabled'], [
            'value' => $request['is_gst_certificate_enabled']
        ]);
        
         DB::table('business_settings')->updateOrInsert(['key' => 'is_pan_card_enabled'], [
            'value' => $request['is_pan_card_enabled']
        ]);
        
         DB::table('business_settings')->updateOrInsert(['key' => 'is_cin_certificate_enabled'], [
            'value' => $request['is_cin_certificate_enabled']
        ]);
        
        DB::table('business_settings')->updateOrInsert(['key' => 'is_abn_certificate_enabled'], [
            'value' => $request['is_abn_certificate_enabled']
        ]);

	DB::table('business_settings')->updateOrInsert(['key' => 'is_tfn_certificate_enabled'], [
            'value' => $request['is_tfn_certificate_enabled']
        ]);
        
         DB::table('business_settings')->updateOrInsert(['key' => 'is_acn_certificate_enabled'], [
            'value' => $request['is_acn_certificate_enabled']
        ]);

	DB::table('business_settings')->updateOrInsert(['key' => 'is_ein_certificate_enabled'], [
            'value' => $request['is_ein_certificate_enabled']
        ]);
        
	DB::table('business_settings')->updateOrInsert(['key' => 'is_gst_certificate_enabled_for_vendor'], [
	  'value' => $request['is_gst_certificate_enabled_for_vendor']
	]);

	DB::table('business_settings')->updateOrInsert(['key' => 'is_fssai_certificate_enabled_for_vendor'], [
	   'value' => $request['is_fssai_certificate_enabled_for_vendor']
	]);

	DB::table('business_settings')->updateOrInsert(['key' => 'is_pan_card_enabled_for_vendor'], [
	  'value' => $request['is_pan_card_enabled_for_vendor']
	]);

	DB::table('business_settings')->updateOrInsert(['key' => 'is_cin_certificate_enabled_for_vendor'], [
	  'value' => $request['is_cin_certificate_enabled_for_vendor']
	]);

	DB::table('business_settings')->updateOrInsert(['key' => 'is_abn_certificate_enabled_for_vendor'], [
	  'value' => $request['is_abn_certificate_enabled_for_vendor']
	]);

	DB::table('business_settings')->updateOrInsert(['key' => 'is_tfn_certificate_enabled_for_vendor'], [
	  'value' => $request['is_tfn_certificate_enabled_for_vendor']
	]);

	DB::table('business_settings')->updateOrInsert(['key' => 'is_acn_certificate_enabled_for_vendor'], [
	  'value' => $request['is_acn_certificate_enabled_for_vendor']
	]);

	DB::table('business_settings')->updateOrInsert(['key' => 'is_ein_certificate_enabled_for_vendor'], [
	 'value' => $request['is_ein_certificate_enabled_for_vendor']
	]);


	DB::table('business_settings')->updateOrInsert(['key' => 'is_bank_cheque_enabled_for_vendor'], [
	  'value' => $request['is_bank_cheque_enabled_for_vendor']
	]);

	DB::table('business_settings')->updateOrInsert(['key' => 'is_pasbook_image_enabled_for_vendor11'], [
	 'value' => $request['is_pasbook_image_enabled_for_vendor11']
	]);
    ...
    public function recycling_policy()
    {
        $data = BusinessSetting::where(['key' => 'recycling_policy'])->first();
        if ($data == false) {
            $data = [
                'key' => 'recycling_policy',
                'value' => '',
            ];
            BusinessSetting::insert($data);
        }
        return view('admin-views.business-settings.recycling-policy', compact('data'));
    }

    public function recycling_policy_update(Request $request)
    {
        BusinessSetting::where(['key' => 'recycling_policy'])->update([
            'value' => $request->recycling_policy,
        ]);

        Toastr::success(translate('Recycling Policy Updated'));
        return back();
    }

    public function return_policy()
    {
        $data = BusinessSetting::where(['key' => 'return_policy'])->first();
        if ($data == false) {
            $data = [
                'key' => 'return_policy',
                'value' => '',
        ...
        return view('admin-views.business-settings.return-policy', compact('data'));
    }

    public function return_policy_update(Request $request)
    {
        BusinessSetting::where(['key' => 'return_policy'])->update([
    ...
    public function shipping_policy_update(Request $request)
    {
        ...
        Toastr::success(translate('Shipping Policy Updated'));
        return back();
    }
