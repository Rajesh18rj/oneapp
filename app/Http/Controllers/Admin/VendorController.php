...
class VendorController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $store->sub_zone_name = $request->sub_zone_name;
    ...
    public function view(Store $store, $tab = null, $sub_tab = 'cash')
    {
        ...
        if ($tab == 'settings') {
            ...
            return view('admin-views.vendor.view.documents', compact('store'));
        }
    ...
    public function module_wise_list(Request $request)
    {
        $zone_id = $request->query('zone_id', 'all');
        $type = $request->query('type', 'all');
        $stores = Store::when(is_numeric($zone_id), function ($query) use ($zone_id) {
            ...
            ->with('vendor', 'module')->type($type)->latest()->paginate(config('default_pagination'));
        $zone = is_numeric($zone_id) ? Zone::findOrFail($zone_id) : null;
        return view('admin-views.vendor.module-wise-list', compact('stores', 'zone', 'type'));
    }
    ...
    {
        $zone_ids = isset($request->zone_ids) ? (count($request->zone_ids) > 0 ? $request->zone_ids : []) : ['1'];
        $data = Store::withOutGlobalScopes()->join('zones', 'zones.id', '=', 'stores.zone_id')
            ...
            ->where(function ($query) {
                // $indiaPolygon = 'POLYGON((68.176645 7.965534, 97.402561 35.494009, 68.842599 35.494009, 68.176645 7.965534))';

             //   $pp = "POLYGON(34.80240123151166 73.64757031250001, 27.904495093497808 68.72569531250001, 23.147795009329858 67.49522656250001, 20.537505172302673 68.02257031250001, 11.078538109874556 74.17491406250001, 7.261517039155428 74.87803906250001, 8.306471323647934 79.09678906250001, 19.38096846415365 86.12803906250001, 23.79274358222831 87.53428906250001, 27.437466207499902 86.47960156250001, 33.34646180938258 79.44835156250001)";

              //  $query->whereRaw("ST_CONTAINS(ST_GEOMFROMTEXT('$pp'), zones.coordinates)");
            })
    ...
    public function bulk_import_data(Request $request)
    {
        ...
        foreach ($collections as $key => $collection) {
            ...
            if ($collection['ownerFirstName'] === "" || $collection['storeName'] === "" || $collection['phone'] === "" || $collection['email'] === "" || $collection['latitude'] === "" || $collection['longitude'] === "" || $collection['zone_id'] === "" || $collection['module_id'] === "" || $collection['gst_number'] === "") {
            ...
            array_push($stores, [
                ...
                'gst_number' => $collection['gst_number'],
                'fssai_number' => $collection['fssai_number'],
                'drug_license_number' => $collection['drug_license_number'],
                'created_at' => now(),
                'updated_at' => now()
            ]);
    ...
    public function bulk_delete(Request $request)
    {
        ...
        Toastr::success(translate('messages.store_deleted_successfully'));

        return json_encode(array('status' => 'true', 'message' => translate('messages.store_deleted_successfully')));
