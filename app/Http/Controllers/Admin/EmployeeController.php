...
class EmployeeController extends Controller
{
    ...
    {
        $rls = AdminRole::whereNotIn('id', [1, 8])->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        $e = Admin::findOrFail($id);
        if ($request['password'] == null) {
            $pass = $e['password'];
        } else {
            if (strlen($request['password']) < 9) {
                Toastr::warning(translate('messages.password_length_warning', ['length' => '9']));
                return back();
            }
    ...
    public function bulk_delete(Request $request)
    {
        ...
        if (isset($itemIds) && !empty($itemIds)) {
            ...
            foreach ($itemIds as $key => $value) {
                ...
                Admin::zone()->where('role_id', '!=', '1')->where(['id' => $value])->delete();
            }
        }
