...
class HomeController extends Controller
{
    ...
    {
        $data = self::get_settings('return_policy');
        if ($request->expectsJson())
        {
            return response()->json($data);
        }
        
        return view('return-policy',compact('data'));
    }
    ...
    public function recycling_policy(Request $request)
    {
        $data = self::get_settings('recycling_policy');
        if ($request->expectsJson())
        {
            return response()->json($data);
        }

        return view('recycling-policy',compact('data'));
    }

    public function reader_policy(Request $request)
    {
        // $data = self::get_settings('reader-policy');
        // if ($request->expectsJson())
        // {
        //     return response()->json($data);
        // }
        return view('reader-policy');
    }
