...
class ItemController extends Controller
{
    public function index()
    {
        ...
        return view('vendor-views.product.index', compact('categories','module_data','module_id'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if(!empty($request->video_url)){
            $images[] = "video-" . $request->video_url;
        }
        ...
        $food->images = $images;
        $food->tax = $request->tax;
        $food->stock = $request->current_stock??0;
        $food->hsn = $request->hsn;
        $food->sac = $request->sac;
        $food->batch_number = $request->batch_number;
        $food->hsn_image = Helpers::upload('product/', 'png', $request->file('hsn_image'));
        $food->save();
    ...
    if(!Helpers::get_store_data()->item_section)
	  ...
	  $product = Item::withoutGlobalScope('translate')->findOrFail($id);
        ...
        $module_data = config('module.'. Helpers::get_store_data()->module->module_type);
        return view('vendor-views.product.edit', compact('product', 'product_category', 'categories','module_data', 'sub_category','category'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($images as $key => $video) {
                if(strpos($video, 'video-') !== false) unset($images[$key]);
        }

        if(!empty($request->video_url)) $images[] = "video-" . $request->video_url;
        ...
        $p->hsn = $request->hsn;
        $p->sac = $request->sac;
        $p->batch_number = $request->batch_number;
        $p->expiry_date = $request->expiry_date;
        $p->hsn_image = $request->has('hsn_image') ? Helpers::update('product/', $p->hsn_image, 'png', $request->file('hsn_image')) : $p->hsn_image;
        $p->save();
    ...
    public function delete(Request $request)
    {
     ...
     public function bulk_delete(Request $request){
		...
		foreach ($itemIds as $key => $value) {
		    ...
		    $product = Item::withoutGlobalScope(StoreScope::class)->withoutGlobalScope('translate')->find($value);
		    ...
		    // Delete translations associated with the product
	   ...
	   // Toastr::success(translate('messages.product_deleted_successfully'));
	     ...
	     return json_encode(array('status' => 'true','message' => translate('messages.product_deleted_successfully')));
    ...
    public function bulk_import_data(Request $request)
    {
        ...
        foreach ($collections as $collection) {
            if ($collection['name'] === "" || $collection['category_id'] === "" || $collection['sub_category_id'] === "" || $collection['price'] === "" || empty($collection['available_time_starts']) === "" || empty($collection['available_time_ends']) || empty($collection['veg']) === ""  || $collection['batch_number'] === "") {
            ...
            array_push($data, [
                ...
                'images' => json_encode([]),
                'hsn' => $collection['hsn']?$collection['hsn']:'',
                'sac' => $collection['sac'] ?? '',
                'tax' => $collection['gst'],
                'batch_number' => $collection['batch_number'],
                'expiry_date' => $collection['expiry_date']->format('Y-m-d'),
