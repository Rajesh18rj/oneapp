...
class DeliverymanController extends Controller
{
    ...
    public function get_current_orders(Request $request)
    {
       ...
       if(isset($orders) && !empty($orders)){
       	foreach($orders as $key => $value){
       	 	...
       	 	$insurance_attachments = json_decode($value['insurance_attachments'],true);
       	 $orders[$key]['insurance_attachments'] = $insurance_attachments;
       	 
       	    	$frieght_attachments = json_decode($value['frieght_attachments'],true);
       	 $orders[$key]['frieght_attachments'] = $frieght_attachments;
       		$order_type = $value['order_type']; 
	       $last_data = DeliveryHistory::whereHas('delivery_man.orders', function ($query) use ($value)  {
		    return $query->where('delivery_man_id', $value['delivery_man_id']);
		})->latest()->first();
		
		if(isset($last_data) && !empty($last_data)){
			$dm_lat = $last_data['latitude'];
		$dm_long = $last_data['longitude'];
		$delivery_address = json_decode($value['delivery_address'], true);
		$order_lat = $delivery_address['latitude'];
		$order_long =  $delivery_address['longitude'];
		$order_delivery_location_limit = $dm['order_delivery_location_limit'];
		
		$distance = Helpers::calculateDistance($dm_lat, $dm_long, $order_lat, $order_long);
		
		
		if ($value['module_type'] == 'food') {
		    $value['is_converted_to_parcel'] = 0;
		} else {
		    if ($order_type == 'parcel') {
			$value['is_converted_to_parcel'] = 0;
		    } else {
			if ($distance >= $order_delivery_location_limit) {
			    $value['is_converted_to_parcel'] = 1;
			} else {
			    $value['is_converted_to_parcel'] = 0;
			}
		    }
    ...
    public function update_tracking_number(Request $request)
    {
        $validator = Validator::make($request->all(), [
    ...
    public function get_order_details(Request $request)
    {
        ...
        foreach ($details as $orderDetail) {
            // Decode the JSON object to an associative array
            $orderDetail = json_decode($orderDetail, true);
            $itemDetail = json_decode($orderDetail['item_details'], true);

            // Check if the 'store_id' matches the desired value (e.g., 30)
            if (isset($itemDetail['store_id']) && $itemDetail['store_id'] == $order->store_id) {
                // If the 'store_id' matches, add it to the filtered array
                $filteredOrderDetails[] = $orderDetail;
            }
        }
        $details = Helpers::order_details_data_formatting($filteredOrderDetails);
...
if(isset($orders) && !empty($orders)){
    ...
    public function remove_account(Request $request)
    {
     ...
     public function add_tracking_data(Request $request){
        $dm =  DeliveryMan::where(['auth_token' => $request['token']])->first();
        $order = Order::where('delivery_man_id', $dm['id'])->where('id', $request['order_id'])->first();
    	...
    	if ($order) {
            ...
            $data = [
                    'title' => 'Order Status',
                    'description' => 'Your order has been converetd to parcel . Please pay amount '.$request['amount'].' to delivery man for this tracking ID '.$request['tracking_number'],
                    'order_id' => $order->id,
                    'image' => '',
                    'type' => 'order_status',
            ];
            ...
            DB::table('user_notifications')->insert([
             ...
             return response()->json(['message' => translate('Details saved successfully')], 200);
    	    
    	} else {
    	
    	  return response()->json(['message' => translate('No delivery man assigned to this order')], 200);
    	}
       
    }
    
    
     public function add_parcel_information(Request $request){
        $dm =  DeliveryMan::where(['auth_token' => $request['token']])->first();
        $order = Order::where('delivery_man_id', $dm['id'])->where('id', $request['order_id'])->first();

    	if ($order) {
    	    $order->cargo_number = $request['cargo_number'];
    	    $order->container_id = $request['container_id'];
    	    $order->insurance_price = $request['insurance_price'];
    	    $order->insurance_number = $request['insurance_number'];
    	    $order->invoice_value = $request['invoice_value'];
    	    if ($request->has('insurance_attachments')) {
	    $attachments = [];
	    foreach ($request->file('insurance_attachments') as $file) {
		$attachments[] = Helpers::upload('order/', 'png', $file);
	    }
	    $order->insurance_attachments = $attachments;
		} else {
		    $order->insurance_attachments = null;
		}
		
		  if ($request->has('frieght_attachments')) {
	    $attachments = [];
	    foreach ($request->file('frieght_attachments') as $file) {
		$attachments[] = Helpers::upload('order/', 'png', $file);
	    }
	    $order->frieght_attachments = $attachments;
		} else {
		    $order->frieght_attachments = null;
		}
            ...
            $data = [
                    'title' => 'Order Info',
                    'description' => 'Delivery man updated like cargo number , container id to your parcel order . Please check',
                    'order_id' => $order->id,
                    'image' => '',
                    'type' => 'order_status',
            ];
