...
class DeliveryManLoginController extends Controller
{
    public function login(Request $request)
    {
        ...
        if (auth('delivery_men')->attempt($data)) {
            ...
            if(auth('delivery_men')->user()->delivery_type == 'individual'  && auth('delivery_men')->user()->added_by != ''){
               if (auth('delivery_men')->user()->approve_status == 'pending') {
		        return response()->json([
		            'errors' => [
		                ['code' => 'auth-003', 'message' => 'Your account is not approved by admin']
		            ]
		        ], 401);
              }
              
              if (auth('delivery_men')->user()->approve_status == 'reject') {
		        return response()->json([
		            'errors' => [
		                ['code' => 'auth-003', 'message' => 'Your account is rejected by admin']
		            ]
		        ], 401);
              }
