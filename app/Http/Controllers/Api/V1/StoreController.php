    ...
    public function get_popular_stores(Request $request)
    {
       ...
       /*  $languageName = $request->header('X-Localization');
            ...
            //$languageName = 'ta';
             ...
             foreach($stores['stores'] as $key => $value){
          
         
		       if($languageName == 'ta'){
		    	   $translatedData = DB::table('translations')->where(array('translationable_type' =>  "App\\Models\\Item", "translationable_id" => $value['id'] , "locale" => "ta"))->first();
		           $stores['stores'][$key]['name'] = (isset($translatedData) && !empty($translatedData)) ? $translatedData->value : ''; 
		       }
		       else if($languageName == 'hi'){
		    	   $translatedData = DB::table('translations')->where(array('translationable_type' =>  "App\\Models\\Item", "translationable_id" => $value['id'] , "locale" => "hi"))->first();
		    	   $stores['stores'][$key]['name'] = (isset($translatedData) && !empty($translatedData)) ? $translatedData->value : '';
		       } else {
		            $translatedData = DB::table('categories')->where(array("id" => $value['id']))->first();
		            $stores['stores'][$key]['name'] = (isset($translatedData) && !empty($translatedData)) ? $translatedData->name : '';
		       }
               
           	}
           	echo "<pre>";
        
        print_r($stores['stores']);
        die;*/
        $stores['stores'] = Helpers::store_data_formatting($stores['stores'], true);
