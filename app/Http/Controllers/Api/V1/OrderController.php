...
class OrderController extends Controller
{
 ...
 public function place_order_new(Request $request)
    ...
    public function place_order(Request $request)
    {
        ...
        if (request()->hasHeader('countryCode')) {
            $countryCode = request()->header('countryCode');
            $currency = Currency::find("country_code", $countryCode)->first();

            $order->exchange_rate = $currency->exchange_rate ?? 1;
            $order->currency_id = $currency->id ?? 1;
        }
        ...
        if ($request->order_type !== 'parcel') {
            ...
            foreach (json_decode($request['cart'], true) as $c) {
                if ($c['item_campaign_id'] != null) {
                    ...
                    if ($product) {
                        ...
                        $product = Helpers::product_data_formatting($product, false, false, app()->getLocale());
                        $addon_data = Helpers::calculate_addon_price(\App\Models\AddOn::whereIn('id', $c['add_on_ids'])->get(), $c['add_on_qtys']);
                        //round(Helpers::tax_calculate($product, $price), config('round_up_to_digit')),
                        $or_d = [
                            'item_id' => $c['item_id'],
            ...
            $order->logistic_type_id = $request['logistic_type_id'];
             $order->commodity_type_id = $request['commodity_type_id'];
            $order->description = $request['description'];
            $vehicle_price = (int)$request['vehicle_price'];
             $order->vehicle_price = $vehicle_price;
             $order->invoice_value = $request['invoice_value'];
            $order->is_insurance_required = $request['is_insurance_required'];
            
            $order->order_amount = round($vehicle_price, config('round_up_to_digit'));
        ...
        try {
            ...
            try {
             ...
             if($request->order_type == 'parcel'){
              $order_amount = round($vehicle_price, config('round_up_to_digit'));
            } else {
    ...
    public function get_order_list(Request $request)
    {
		...
		$order_delivery_location_limit = $dm['order_delivery_location_limit'] ?? 0;
