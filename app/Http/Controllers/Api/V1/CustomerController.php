...
class CustomerController extends Controller
{
    ...
    public function add_new_address(Request $request)
    {
        ...
        return response()->json(['message' => translate('messages.successfully_added'),'zone_id'=>$zone->id], 200);
    }
    ...
    function downloadInvoice(Request $request){
        $orderId = $request->get('invoice_id'); 
        $order = Order::with(['details', 'store' => function ($query) {
         ...
         if (isset($order)) { 
           ...
           $pdf = PDF::setOptions(['debugKeepTemp' => true,'defaultFont' => 'sans-serif','isHtml5ParserEnabled' => true, 'isPhpEnabled' => true])->loadView('order-invoice',compact('order','logoPath'));
   ...
   public function make_parcel_payment(Request $request){
        $token = request()->header('Authorization');
     
        $user_id = $request['user_id'];

       // $dm =  User::where(['cm_firebase_token' => $token])->first();
        $order = Order::where('user_id', $user_id)->where('id', $request['order_id'])->first();

    	if ($order) {
    		
    	    //$order->amount = $request['amount'];
    	    $order->transaction_id = $request['transaction_id'];
    	    $order->parcel_payment_status = 'paid';
            $order->save();
            /*$data = [
                    'title' => 'Order Status',
                    'description' => 'Your order has been reached at courier service center . Please pay amount '.$request['amount'].' to delivery man for your order to be delivered',
                    'order_id' => $order->id,
                    'image' => '',
                    'type' => 'order_status',
            ];
    ...
    public function pay_insurance_amount(Request $request){
        $token = request()->header('Authorization');
       ...
       // $dm =  User::where(['cm_firebase_token' => $token])->first();
    	...
    	if ($order) {
    	    ...
    	    $order_insurance_price = $order->insurance_price;
    	    
    
    	    if($order_insurance_price != $insurance_price){
    	        return response()->json(['message' => "Please pay your correct inusrance amount " .$order_insurance_price], 200);
    	    }
    	
    		
    	    //$order->amount = $request['amount'];
    	    $order->insurance_transaction_id = $request['insurance_transaction_id'];
    	    $order->insurance_payment_status = 'paid';
            $order->save();
            /*$data = [
                    'title' => 'Order Status',
                    'description' => 'Your order has been reached at courier service center . Please pay amount '.$request['amount'].' to delivery man for your order to be delivered',
        ...
        return response()->json(['message' => translate('Amount paid successfully')], 200);
    }  
}
