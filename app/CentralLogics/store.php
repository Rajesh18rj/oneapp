...
class StoreLogic
{
    public static function get_stores( $zone_id, $filter, $type, $limit = 10, $offset = 1, $featured=false)
    {
        ...
        if(config('module.current_module_data')) {
            $paginator = $paginator->module(config('module.current_module_data')['id'])
    ...
    public static function get_latest_stores($zone_id, $limit = 10, $offset = 1, $type='all')
    {
        ...
        ->when(config('module.current_module_data'), function($query)use($zone_id){
            $query->module(config('module.current_module_data'));
    ...
    public static function format_export_stores($stores)
    {
        ...
        foreach($stores as $item)
        {
            ...
            $storage[] = [
                ...
                'gst_number' => $item->stores[0]->gst_number,
                'fssai_number' => $item->stores[0]->fssai_number,
                'drug_license_number' => $item->stores[0]->drug_license_number,
            ];
        }
